...
const (
	...
	Not_Send_Merge     NeedSendStatus = iota
	Not_Send_Setback   NeedSendStatus = iota
...
// Not_Send_Failed_CR - doc does not need to be sent to target since it failed source side conflict resolution (target wins)
// Not_Send_Merge - for CCR only, the documents need to be merged
// Not_Send_SetBack - for CCR only, the document needs to be sent back to source cluster
